...
def test_dataset(): 
  ...
  for _, data in enumerate(sample_dataset):
    ...
    print(i)
    print(data['statement'].numpy().dtype)
    print(correct_statements[i].dtype)
    '''
    for j in range(0, len(data['statement'])):
      print(data['statement'][j].numpy())
      print(correct_statements[i][j])
      if data['statement'][j]==correct_statements[i][j]:
        print('equal')
      else:
        print('not equal')
    '''
