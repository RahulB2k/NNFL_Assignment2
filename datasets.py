    ...
    def __getitem__(self, idx): # 1.25 Mark
        ...
        for pos, word in enumerate(justification):
            try:
                ...
                print('Word not in Vocab. Placing it at origin')
                continue
        tensor_justification=torch.tensor(vectorized_justification)
        tensor_statement=torch.tensor(vectorized_statement)
        credit=self.dataframe.iloc[idx,9:14].to_numpy(dtype='float32')
        credit_history=numpy.zeros(5, dtype='float32')
        credit_history[0]=credit[3]
        credit_history[1]=credit[2]
        credit_history[2]=credit[0]
        credit_history[3]=credit[1]
        credit_history[4]=credit[4]
        tensor_credit_history=torch.tensor(credit_history)
        data_dict['statement']=tensor_statement
        data_dict['justification']=tensor_justification
        data_dict['label']=tensor_label
        data_dict['credit_history']=tensor_credit_history
